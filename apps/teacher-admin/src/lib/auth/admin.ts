...
// Admin gatekeeper, modeled on Handwritten-Assignment-Helper's
// getCurrentAdminEmail. Returns the logged-in user's email iff they have an
...
// callers (like /admin/layout.tsx) decide what to do (redirect, hide UI, etc).
...
export const getCurrentAdminEmail = cache(async (): Promise<string | null> => {
  ...
  // admins table is currently empty. Idempotent — once any admin row exists,
  // bootstrap won't fire.
