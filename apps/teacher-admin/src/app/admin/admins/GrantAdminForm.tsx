...
export function GrantAdminForm({
  ...
  function onGrant() {
    ...
    startTransition(async () => {
      ...
      if (r.ok) {
        setFeedback({ kind: "ok", message: `${email} can now sign in to /admin.` });
        setEmail("");
      } else {
  ...
  return (
    <section className="rounded-md border border-stone-200 bg-white p-4">
      ...
      <p className="mb-3 text-xs text-stone-500">
        Email must match the user&apos;s EHS Google account exactly
        {requiredDomain && (
          <>
            {" "}
            (must be on{" "}
            <span className="font-mono text-stone-700">@{requiredDomain}</span>)
          </>
        )}
        .
      </p>
