...
export function NewPromptForm() {
  ...
  function onCreate() {
    ...
    startTransition(async () => {
      const r = await createPrompt({ label, body, studentFacingQuestion });
      if (r.ok) {
        setOpen(false);
        reset();
      } else {
  ...
  if (!open) {
    return (
      <button
        ...
        className="rounded-sm border border-stone-300 bg-white px-3 py-1.5 text-sm font-medium text-stone-700 shadow-sm hover:border-maroon/50 hover:text-maroon"
      >
        + New prompt
      </button>
    );
  ...
  return (
    <section className="rounded-sm border border-stone-300 bg-white p-4 shadow-sm">
      <h2 className="mb-3 text-sm font-semibold text-stone-900">New prompt</h2>
      <div className="space-y-3">
        <input
          ...
          spellCheck={false}
          className="w-full rounded-sm border border-stone-300 bg-white px-3 py-1.5 text-sm focus:border-maroon"
        />
        ...
        <div>
          ...
          <textarea
            ...
            rows={12}
            spellCheck={false}
            className="w-full resize-y rounded-sm border border-stone-300 bg-white px-3 py-2 font-mono text-xs leading-relaxed focus:border-maroon"
          />
        </div>
      </div>
      <div className="mt-3 flex items-center gap-2">
        <button
          ...
          className="rounded-sm bg-maroon px-3 py-1.5 text-sm font-semibold text-white hover:bg-maroon-dark disabled:cursor-not-allowed disabled:bg-stone-300 disabled:text-stone-500"
        >
